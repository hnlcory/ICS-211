...
public class ArraySorter<E> implements SortsArray<E> {
  ...
  public void insertionSort(Comparator<E> compare, SortRecord<E>[] data) {
    ...
    for (int fill = 0; fill < n - 1; fill++) {
      ...
      for (int next = fill + 1; next < n; next++) {
        ...
        int result = compare.compare(data[next].element, data[posMin].element); // table[next].compareTo(table[posMin])
  ...
  /**
   * @author Cory Parker.
   *  Used http://courses.ics.hawaii.edu/ics211s21/morea/060.simple-sorting/experience-sorting-practice.html for guide.
   */
  @Override
  public void bubbleSort(Comparator<E> compare, SortRecord<E>[] data) {
    ...
    do { // do while loop
      ...
      for (int i = 1; i < data.length - pass; i++) {// loop from 0 to data.length -1 - number of passes
        ...
        int result = compare.compare(data[i].element, data[i - 1].element);// compare data[i].element,
                                                                           // data[i+1].element, if data > 0, swap,
                                                                           // count swap
  ...
  public void selectionSort(Comparator<E> compare, SortRecord<E>[] data) {// works sometimes?
    ...
    for (int i = 0; i < data.length - 1; i++) {// loop from 0 to data.length -1
      ...
      for (int j = i + 1; j < data.length; j++) {// orignally data.length-1 triggered a sometimes working? removed and
                                                 // works always?
